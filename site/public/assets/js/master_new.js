...
var i = 0;
    ...
    function openModal(data){
        jQuery('#modal-body').html('<div>'+data.content+'</div>')
        jQuery('#modal-box').attr('class',data.modalclass)

        jQuery('#modal').attr('class','modal-on')
        if(data.action!=null&&data.action!=undefined){
            jQuery('#modal-btn').attr('onclick',data.action)
            jQuery('#modal-btn').html(data.value)
            jQuery('#modal-btn').attr('class',data.btnclass)
        }else{
            jQuery('#modal-btn').attr('onclick','closeModal()')
        }
    }
    function closeModal(){
        jQuery('#modal').attr('class','modal-off')
    }
    ...
    function fillDepartamentos(){
        ...
        jQuery('#departamento_residencia_txt').html(html)
        jQuery('#departamento_residencia_txt').val('0')
    }
    ...
    function fillSectores(){
        ...
        jQuery('#sector_txt').html(html)
        jQuery('#sector_txt').val('0')
    }
    ...
    function cambiarDepartamento(select){
        if(select.value!='0'){
            ...
            jQuery('#ciudad_residencia_txt').html(html)
            jQuery('#ciudad_residencia_txt').val('0')
        }
    ...
    function borrarArchivo(call){
        jQuery('#archivo_form').trigger("reset");
    ...
    function validarNit(campo,validate){
        var longitud = String(campo.value)
        var tipo_doc = jQuery('#tipo_documento_txt').val()
        var padre = campo.parentNode
        var help_txt = padre.getElementsByClassName('form-help')[0]
        if(longitud.length>9){
            help_txt.style.display = 'block'
            help_txt.innerHTML = 'Recuerde no incluir el dígito de verificación del NIT'
        }else{
    ...
    function validateForm(save,id){
        ...
        var numero_documento_txt = jQuery('#numero_documento_txt').val()
        ...
        var tipo_documento_txt = jQuery('#tipo_documento_txt').val()
        ...
        var numero_telefonico_txt = jQuery('#numero_telefonico_txt').val()
        ...
        var correo_electronico_txt = jQuery('#correo_electronico_txt').val()
        ...
        var sector_txt = jQuery('#sector_txt').val()
        if(sector_txt=='0'){
        ...
        var departamento_residencia_txt = jQuery('#departamento_residencia_txt').val()
        ...
        var ciudad_residencia_txt = jQuery('#ciudad_residencia_txt').val()
        ...
        var direccion_txt = jQuery('#direccion_txt').val()
        ...
        var politicas_txt = jQuery('#politicas_txt').prop('checked')
        if(!politicas_txt){
        ...
        var file_txt = jQuery('#archivo_txt').val()
        if(empty(file_txt)){
        ...
        if(errors.length==0){
            //habilitar boton
            jQuery(guardar_btn).attr('class','btn-unlocked')
            guardar_btn.disabled = false
        }else{
            //deshabilitar
            jQuery(guardar_btn).attr('class','btn-locked')
    ...
    function clickGuardar(btn){
        ...
        var ciudad_residencia_txt = jQuery('#ciudad_residencia_txt').val()
        var direccion_txt = jQuery('#direccion_txt').val()
        var archivo_txt = jQuery('#archivo_txt').val()
        ...
        jQuery('#nombre_legal_value').html(clearVal(nombre_legal_txt))
        jQuery('#nombre_comercial_value').html(clearVal(nombre_comercial_txt))
        jQuery('#documento_value').html(clearVal(tipo_documento_txt)+'. '+clearVal(numero_documento_txt))

        jQuery('#numero_telefonico_value').html(clearVal(numero_telefonico_txt))
        jQuery('#correo_electronico_value').html(clearVal(correo_electronico_txt))

        jQuery('#sector_value').html(clearVal(sectores_data[sector_txt]))
                        
        jQuery('#departamento_residencia_value').html(ciudades_data[parseInt(departamento_residencia_txt-1)].departamento)
        jQuery('#ciudad_residencia_value').html(ciudades_data[parseInt(departamento_residencia_txt-1)].municipios[ciudad_residencia_txt-1].municipio)
        jQuery('#direccion_value').html(clearVal(direccion_txt))
        
        if(politicas_txt){
            jQuery('#politicas_value').addClass('form-checkbox-square-checked')
        }

        jQuery('#archivo_value').html(archivo_name_txt+'.'+archivo_ext_txt)

        jQuery('#actualizacion-datos-formulario').attr('style','display:none')
        jQuery('#actualizacion-datos-formulario2').attr('style','display:block')
        jQuery("html, body").animate({ scrollTop: jQuery('#actualizacion-datos-formulario2').offset().top }, 500);
    }

    function clickRegresar(btn){
        jQuery('#actualizacion-datos-formulario').attr('style','display:block')
        jQuery('#actualizacion-datos-formulario2').attr('style','display:none')
        jQuery("html, body").animate({ scrollTop: jQuery('#actualizacion-datos-formulario').offset().top }, 500);
    }


    function clickConfirmar(btn){
        jQuery('#regresar-btn').disabled = true
        jQuery('#correctos-btn').disabled = true
        jQuery('#correctos-btn').html('... ENVIANDO')
        ...
        //subir archivo
        var file_td = jQuery('#file_td').val(tipo_documento_txt)
        var file_nd = jQuery('#file_nd').val(numero_documento_txt)
        
        jQuery("#archivo_form").ajaxForm({
                ...
                if(result_json.success=='success'&&result_json.code=='201'){
                    ...
                    jQuery.ajax({
                        ...
                        success: function(result){
                            ...
                            jQuery('#regresar-btn').disabled = true
                            jQuery('#correctos-btn').disabled = true
                            jQuery('#correctos-btn').html('Están correctos')
                        },
                        error: function(xhr){
                            console.log(xhr)
                            jQuery('#regresar-btn').disabled = true
